...
int exec_bytecode() {
    int program[] = {0x01010203, 0x02030405, /* more instructions */};
    int program_size = sizeof(program) / sizeof(program[0]);

    int registers[256] = {0};  // this array holds the register values
    int pc = 0;  // Program counter
    int running = 1;

    while (running) {
        // checks if pc is permissable
        if (pc >= program_size) {
            running = 0;  // stops the exec
            break;
        }

        // fetches instruction from program
        int instruction = program[pc];

        // extracts opcode and func fields
        int opcode = (instruction >> 24) & 0xFF;
        int func = (instruction >> 16) & 0xFF;

        // execs instructions based of opcode and func
        if (opcode == 0x01) {
            // adds 2 values
            int reg1 = (instruction >> 8) & 0xFF;
    ...
    // returns value of last register
    return registers[255];
}
